...
#you got a warning message fitting dipper.phi.t.p.t, check to see if you know
#what it means and what it means for inference. Is it ok to fit models where variance calculation
...
abline(h=predict(dipper.const)$phi[2,4],lwd=1, lty=2) #lower 95% CI for phi (survival probability)
abline(h=predict(dipper.const)$phi[2,5],lwd=1, lty=2) #upper 95% CI for phi  (survival probability)
...
# if you feel like you don't really know what the set of good models *really* are and/or you
#more interested in the parameter estimates (e.g. for prediction) you can can do model averaging, although
